...
static int c3_conv_bn_silu_int8(conv2d_layer_t* conv, batchnorm2d_layer_t* bn, int fused,
...
int c3_forward_float(c3_block_t* block, const tensor_t* input, tensor_t* output,
    ...
    if (c3_conv_bn_silu_int8(&block->cv1, &block->cv1_bn, block->cv1_is_fused, input, workspace1) != 0) {
        if (need_free_ws1) tensor_free(workspace1);
        if (need_free_ws2) tensor_free(workspace2);
        return -1;
    }
    ...
    if (c3_conv_bn_silu_int8(&block->cv2, &block->cv2_bn, block->cv2_is_fused, input, skip_output) != 0) {
        tensor_free(skip_output);
        if (need_free_ws1) tensor_free(workspace1);
        if (need_free_ws2) tensor_free(workspace2);
        return -1;
    ...
    if (c3_conv_bn_silu_int8(&block->cv3, &block->cv3_bn, block->cv3_is_fused, workspace2, output) != 0) {
        if (need_free_ws1) tensor_free(workspace1);
        if (need_free_ws2) tensor_free(workspace2);
        return -1;
    }
